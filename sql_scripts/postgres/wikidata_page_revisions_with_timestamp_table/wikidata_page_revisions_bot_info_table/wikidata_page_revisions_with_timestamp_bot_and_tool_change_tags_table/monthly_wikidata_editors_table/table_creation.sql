CREATE TABLE monthly_wikidata_editors AS(
		...
		(
			...
			(
				...
				WHERE (bot_user_id IS NULL AND revision_user NOT LIKE '%.%' AND NOT (lower(regexp_replace(comment, '\.|,|\(|\)|-|:','','g')) LIKE '%quickstatements%' OR 
																					...
																					lower(regexp_replace(comment, '\.|,|\(|\)|-|:','','g')) LIKE '%[[mediawikigadgetmergejs|mergejs]]%' OR
																					revision_user = '2769139'))
	...
	(
		...
		(
			...
			OR change_tag_revision_id IS NOT NULL OR revision_user = '2769139')
			GROUP BY year, month
