	...
	(
		...
		(
			...
			(
				...
				FROM wikidata_page_revisions_bot_info
				WHERE bot_user_id IS NULL AND revision_user NOT LIKE '%.%' AND NOT (comment LIKE '%quickstatements%' OR comment LIKE '%petscan%' OR comment LIKE '%autolist2%')
				GROUP BY page_title
		...
		(
			...
			FROM wikidata_page_revisions_bot_info
			WHERE bot_user_id IS NULL AND revision_user LIKE '%.%' AND NOT (comment LIKE '%quickstatements%' OR comment LIKE '%petscan%' OR comment LIKE '%autolist2%')
			GROUP BY page_title
		) AS anons
