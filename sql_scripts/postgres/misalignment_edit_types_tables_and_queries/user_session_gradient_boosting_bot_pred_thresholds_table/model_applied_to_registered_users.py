"""
Apply previously generated model to registered user data.



Usage:
    model_applied_to_registered_users (-h|--help)
    model_applied_to_registered_users <input_training> <input_testing> <input_registered_data> <r_forest_predictions_output> <gradient_b_predictions_output> <gradient_b_threshold_scores_output> <gradient_b_threshold_scores_i2_output> <testing_output> <pr_output> <roc_output>
                                      [--debug]
                                      [--verbose]
...
Options:
    ...
    <input_training>                         Path to input training file to 
                                             process.
    <input_testing>                          Path to input registered user data
                                             file to process.
    <input_registered_data>                   Path to input registered user data 
                                             file to process.
...
def main(argv=None):
    ...
    run(input_training_file, input_testing_file, input_registered_data_file, 
...
def run(input_training_file, input_testing_file, input_registered_data_file, 
    ...
    registered_predictors_i2 = []
    registered_user_and_session_start = []
    ...
    for i, line in enumerate(input_registered_data_file):
        registered_user_and_session_start.append({'user' : line['user'], 
                                                 'session_start' : 
                                                    line['session_start']})


        registered_predictors.append([line['mean_in_seconds'],
                                   line['std_in_seconds'],
                                   line['namespace_0_edits'],
                                   line['namespace_1_edits'],
                                   line['namespace_2_edits'],
    ...
    threshold_scores =\
        gradient_b_i2_fitted_model.decision_function(registered_predictors_i2)
    ...
    for i, line in enumerate(registered_predictors_i2):

        r_forest_predictions_output_file.write(
            [registered_user_and_session_start[i]['user'],
             registered_user_and_session_start[i]['session_start'],
             line[0],
             line[1],
             line[2],
             line[3],
