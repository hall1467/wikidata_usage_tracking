...
def run(input_files, revisions_output_file, verbose):
    ...
    i = 0
    for title, page_id, revision_id, user_id, user_text, comment, namespace,\
        timestamp in mwxml.map(process_pages, input_files):
        i += 1
        revisions_output_file.write([title, page_id, revision_id, user_id, 
            user_text, comment, namespace, timestamp])
