...
testing and the one used for iteration 2 testing and the new one for R and R.
Returns only test data that is not also used to build the model or for 
iteration 1 or 2 testing.



Usage:
    take_out_used_data_from_testing_set_R (-h|--help)
    take_out_used_data_from_testing_set_R <input_building_data> <input_i1_testing_data> <input_i2_testing_data> <input_r_testing_data> <output>
                                          [--debug]
                                          [--verbose]
...
Options:
    ...
    <input_i2_testing_data>  Path to input iteration 2 testing data file to 
                             process.
    <input_r_testing_data>   Path to input R and R testing data file to 
                             process.                          
...
def main(argv=None):
    ...
    input_r_testing_data_file = mysqltsv.Reader(
        open(args['<input_r_testing_data>'],'rt'), headers=False, 
        types=[str, str, str, str, str])
    ...
    run(input_building_data_file, input_i1_testing_data_file, 
        input_i2_testing_data_file, input_r_testing_data_file, 
        output_file, verbose)
...
def run(input_building_data_file, input_i1_testing_data_file, 
    ...
    random_i2_testing_sessions = defaultdict(lambda: defaultdict(int))
    ...
    for i, line in enumerate(input_i2_testing_data_file):
        random_i2_testing_sessions[line[0]][line[1]] = 1
        
        if verbose and i % 10000 == 0 and i != 0:
            sys.stderr.write("I2 testing sessions returned: {0}\n".format(i))  
            sys.stderr.flush()
    ...
    for i, line in enumerate(input_r_testing_data_file):
        ...
        if (line[0] in random_building_sessions and\
            ...
            line[1] in random_i1_testing_sessions[line[0]]) or\
            (line[0] in random_i2_testing_sessions and\
            line[1] in random_i2_testing_sessions[line[0]]):
