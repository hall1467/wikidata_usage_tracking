...
def run(input_file, output_file, verbose):
    ...
    for i, line in enumerate(input_file):
        ...
        # Range after
        for revision_id, user_id, timestamp, namespace in request_function_and_timestamp_adjusting(cast_timestamp, incremented_cast_timestamp, line, session_for_nearby_revisions):
        ...
        # Range before
        for revision_id, user_id, timestamp, namespace in request_function_and_timestamp_adjusting(decremented_cast_timestamp, cast_timestamp, line, session_for_nearby_revisions):
            unique_revision_ids[timestamp][revision_id]["user_id"] = user_id
            unique_revision_ids[timestamp][revision_id]["edit_type"] = line["edit_type"]
            unique_revision_ids[timestamp][revision_id]["namespace"] = namespace
...
def request_function_and_timestamp_adjusting(cast_timestamp, 
    later_cast_timestamp, line, session_for_nearby_revisions):
    for user_and_timestamp in session_for_nearby_revisions.get(action="query", format="json", list="usercontribs", uclimit="max", ucstart=cast_timestamp, 
        ucend=later_cast_timestamp, ucdir="newer", ucprop=["timestamp","ids", "title"], ucuserids=[line["user_id"]], continuation=True):
