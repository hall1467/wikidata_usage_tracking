...
# Return wikidata object page usages in descending order of usages, with aspect usage information
def getSortedObjectUsagesFromDump(wiki, date):
	...
	return sortUsages(wikidata_object_page_usage_count, wikidata_object_aspect_usage)
...
def sortUsages(object_page_usage_count, object_aspect_usage):
	sorted_object_page_usage_count = sorted(object_page_usage_count.items(), key=operator.itemgetter(1), reverse=True)
	wikidata_usages = []
	for wikidata_object in sorted_object_page_usage_count:
		wikidata_usages.append({'wikidata_object' : wikidata_object[0], 'wiki_pages_used_by' : wikidata_object[1], 'aspects_used_by_pages' : object_aspect_usage[wikidata_object[0]]})
...
def writeWikidataUsagesToFile(directory, object_type, wiki, wikidata_usages, date):
	wiki_file = open(directory + "/" + wiki + "_" + object_type + "_wikidata_usages_" + date + ".csv", "w")
